...
group_CSSE = ProvinceUsesOnly.sort_values(['Deaths','Province_State','Country_Region'],ascending=False).reset_index()
...
filtering = ProvinceUsesOnly[ProvinceUsesOnly['Province_State'].isin(['California'])]
sns.violinplot(x='Province_State', y='New_Deaths', data = filtering)
# %%
...
group_CSSE = ProvinceUsesOnly[['Province_State','Confirmed']].groupby('Province_State').max().reset_index().sort_values('Confirmed',ascending=False)
...
group_CSSE = CountryUsesOnly[['Country_Region','Deaths']].groupby('Country_Region').max().reset_index().sort_values("Deaths",ascending=False)
...
# %%
# group by country region
group_CSSE = CountryUsesOnly[['Province_State','Deaths']].groupby('Country_Region').max().reset_index().sort_values("Deaths",ascending=False)
